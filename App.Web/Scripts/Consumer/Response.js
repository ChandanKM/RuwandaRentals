...
$("#aresponse").click(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            // lmk admin
            var rcvr = "info@lastminutekeys.com";
            $.ajax({
                type: "POST",
                url: "/api/Consumer/SendMail",
                data: { Cons_Subject: sub, Cons_Body: bdy, Cons_mailid: rcvr },
            ...
            //hotel mail sending
            var tot = data.Table[0].camo_room_rate * (data.Table[0].Room_Count * data.Table[0].Days_Count);
            var bdyvndr = '<div style="width:45%;margin:0 auto;font-size:14px;color:#222;line-height:1.6em;font-family:"segoe UI";"><div style="background:url(http://www.lastminutekeys.com/img/Mailer/header-banner.png) repeat-x;padding:12px;background-size: cover;background-position: 100% 100%;"><a href="#" style="display:inline-block;"><img src="http://www.lastminutekeys.com/img/Mailer/logo.png" title="Last Minute Keys" alt="" style="display:block;" /></a> </div><p style="color:#565656;margin:0px auto;padding:15px;font-size: 14px;line-height: 1.6em;box-shadow: 0 1px 1px rgba(0, 0, 0, 0.2);-moz-box-shadow: 0 1px 1px rgba(0, 0, 0, 0.2);-o-box-shadow: 0 1px 1px rgba(0, 0, 0, 0.2);">';
            bdyvndr += '<div style="width:100%;margin:0 auto;font-family: Segoe UI, Tahoma, Geneva, Verdana, sans-serif;font-size:14px;color:#353535;">'
            bdyvndr += 'Your reservation is now confirmed.</p>'
            bdyvndr += '<div><strong>Hotel Name:</strong>&nbsp;&nbsp;&nbsp; <span style="color:#2781CD;font-weight: bold;font-style: italic;margin:0;font-size:15px;">' + data.Table[0].Prop_Name + '</span></div><br>'
            bdyvndr += '<div><strong>Address:</strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <ul style="font-size:14px;line-height:1.8em;color:#565656;list-style-type:none;padding:0;margin-top:0;display:inline-block;vertical-align:top;"><li>' + data.Table[0].Prop_Addr1 + '</li><li> Phone: <strong>022 6668 1234</strong></li></ul></div>'
            bdyvndr += '<br/>'
            bdyvndr += '<table width="100%" border="1" bordercolor="#eee" cellspacing="1" cellpadding="5">'
            bdyvndr += '<tr><td colspan="4" rowspan="1" align="center" bgcolor="#2781cd"><h2 style="font-size: 20px;color: #FFF;margin: 5px 0;font-weight: 600;">Reservation Confirmation</h2></td></tr>'
            bdyvndr += '<tr><td style="width:200px;"> <span style="font-weight: bold;">Occupant Names</span></td><td colspan="3" rowspan="1">' + data.Table[0].GuestName + '</td></tr>'
            bdyvndr += '<tr><td><strong>Booking ID</strong></td><td colspan="3" rowspan="1">' + data.Table[0].Invce_Num + '</td></tr>'
            bdyvndr += '<tr><td><strong>Check-in Date</strong></td><td>' + data.Table[0].Checkin + '</td>'
            bdyvndr += '<td><strong>Check-out Date</strong></td><td>' + data.Table[0].Checkout + '</td></tr>'
            bdyvndr += '<tr><td><strong>Room Type </strong></td><td>' + data.Table[0].Room_Name + '</td><td><strong>No of Rooms</strong></td><td>' + data.Table[0].Room_Count + '</td></tr>'
            bdyvndr += '<tr><td><strong>No of Nights</strong></td><td>' + data.Table[0].Days_Count + '</td><td><strong>Number of Guests</strong></td><td colspan="3" rowspan="1">' + data.Table[0].Room_Count + '</td></tr>'
            bdyvndr += '</table>'
            bdyvndr += '<br />'
            bdyvndr += '<table width="100%" border="1" bordercolor="#eee" cellspacing="1" cellpadding="5">'
            bdyvndr += '<tr><td colspan="2" bgcolor="#eee" align="center"><h4 style="font-size: 18px;color: #42b760;text-transform: uppercase;margin:0px;font-weight:600;">Summary Of Charges</h4></td></tr>'
            bdyvndr += '<tr><th align="left">Room Cost</th><td>₹ <span>' + data.Table[0].camo_room_rate + '</span></td></tr>'
            bdyvndr += '<tr><th align="left">Number of Room</th><td><span>' + data.Table[0].Room_Count + '</span></td></tr>'
            bdyvndr += '<tr><th align="left">Nights</th><td><span>' + data.Table[0].Days_Count + '</span></td></tr>'
            bdyvndr += '<tr><th align="left">Total</th><td>₹ <span>' + tot + '</span></td></tr>'
            bdyvndr += '<tr><th align="left"> Taxes</th><td>₹ <span>' + data.Table[0].tax_amnt + '</span></td></tr>'
            bdyvndr += '<tr><th align="left">Price Payable</th><td style="font-size: 24px;color: #42b760;"><strong>₹ <span>' + data.Table[0].net_amt + '</span></strong> </td></tr>'
            bdyvndr += '</table>'
            bdyvndr += '<br/>'
            bdyvndr += '<p><strong><font color="red">IMP NOTE:</font></strong> - Please carry Photo ID proof along with a photocopy for the tour.</p>'
            bdyvndr += '<p><strong>Need help with your reservation?</strong></p><ul style="font-size:14px;line-height:1.8em;color:#353535;list-style-type:none;padding:0;"><li>Contact LMK&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Phone: +912243438150</li><li> Contact the property &nbsp;&nbsp;&nbsp;&nbsp;Phone: +912223462244</li></ul>'
            bdyvndr += '<br/><br />'
            bdyvndr += '<div style="text-align:right;border-top:3px double #eee;padding-top:20px;"><strong>Thanks & Regards</strong>,<br><font size="3">Lastminutekeys.com</font></div>'
            bdyvndr += '<br/>'
            bdyvndr += '<div style="background:#192b3e;padding:10px;"><h4 style="margin: 0;text-align: center;color: #FFFFFF;font-weight: 400;font-size: 15px;text-transform: uppercase;">Follow Us</h4><ul style="padding:0;list-style:none;border-bottom:1px solid #FFF;border-top:1px solid #FFF;margin:5px 0;padding:5px 0;"><li style="float:left;width:33.3%;text-align:left;margin-left:0;"><a href="#" style="display:block;color:#F5F5F5;font-size:12px;text-decoration:none;"><i style="width:15px;height:15px;background:url(img/Mailer/social-icons.png) no-repeat -2px -6px;display:inline-block;vertical-align: sub;"></i> Follow us on Twitter</a></li><li style="float:left;width:33.3%;text-align:center;margin-left:0;"><a href="#" style="display:block;color:#F5F5F5;font-size:12px;text-decoration:none;"><i style="width:15px;height:15px;background:url(img/Mailer/social-icons.png) no-repeat -24px -5px;display:inline-block;vertical-align: sub;"></i> Follow us on Facebook</a></li><li style="float:left;width:33.3%;text-align:right;margin-left:0;"><a href="#" style="display:block;color:#F5F5F5;font-size:12px;text-decoration:none;"><i style="width:15px;height:15px;background:url(img/Mailer/social-icons.png) no-repeat -53px -5px;display:inline-block;vertical-align: sub;"></i> Follow us on Instagram</a></li><div style="clear:both;"></div></ul><h4 style="margin: 0;text-align: center;color: #FFFFFF;font-weight: 400;font-size: 15px;">Contact OurCustomer Care</h4><p style="color:#fefefe;text-align:center;font-size: 12px;margin:0;font-style:italic;padding:4px 0;"><a href="www.lastminutekeys.com/Home/Contact_Us" style="color:#FFF;font-size:14px;font-weight:400;">Click here</a></p><p style="color:#F4F4F4;text-align:center;font-size: 12px;line-height: 1.6em;margin:10px 0;"></p></div></div></div>'
            rcvrvndr = data.Table[0].prop_booking_mailid

            //SMS Variables
            var subsms = 'Booking Confirmation';
            var bdysms = "Thanks for booking with LMK. Your Booking is confirmed. Booking ID : " + data.Table[0].Invce_Id + " Chk in: " + smsdate + " for " + data.Table[0].Days_Count + " night at " + data.Table[0].Prop_Name + "";
            var Mobile = data.Table[0].Cons_Mobile;
            var sms_gateway = data.Table6[0].Sms_Url;

            $.ajax({
                type: "POST",
                url: "/api/Consumer/SendMail",
                data: { Cons_Subject: sub, Cons_Body: bdyvndr, Cons_mailid: rcvrvndr },
                dataType: "json",
                async: false,
                success: function (response) {
