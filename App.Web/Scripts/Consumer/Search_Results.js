...
var dtto = $.localStorage("CheckOutDate").split('/');
var date2 = dtto[0];
$('#hdnLocationId').val($.localStorage("City_Id"));
$('#txtLocation').val($.localStorage("Location"));
$('#txtCheckIn').val($.localStorage("CheckInDate"));
var dtcurrent = new Date();
var dtcurnt1 = dtcurrent.getDate();
var diff = date2 - dtcurnt1;
$('#txtCheckOut').val(GetDate(diff));
var minDate = $("#txtCheckIn").datepicker('getDate');
var maxDate = $("#txtCheckOut").datepicker('getDate');
...
$("#txtCheckIn").datepicker({    
    ...
    onSelect: function () {
        ...
        //   $('#txtDayCount').val(days + " Night");
        NoOfNight = days; //for store the no. of Night
        if (maxDate.getDate() - (minDate.getDate() - 1) == 1) {
            //$('#txtCheckOut').val(GetDate(3));
            var newMax = new Date(minDate.setDate(minDate.getDate() + 1));
            $("#txtCheckOut").datepicker("option", "maxDate", newMax);
        }
        else if (maxDate.getDate() - (minDate.getDate() - 1) == 0) {
            //$('#txtCheckOut').val(GetDate(3));
            var newMax = new Date(minDate.setDate(minDate.getDate() + 1));
            $("#txtCheckOut").datepicker("option", "maxDate", newMax);
        }
...
if (maxDate.getDate() - (minDate.getDate() - 1) == 1) {
    ...
    //$('#txtCheckOut').val(GetDate(3));
    var newMax = new Date(minDate.setDate(minDate.getDate() + 2));
    $("#txtCheckOut").datepicker("option", "maxDate", newMax);
}
else {
    var newMax = new Date(minDate.setDate(minDate.getDate() + 1));
...
function HotelListModel(data, NoRoomType, rating, Numberreviews, ReviewUrl) {
    ...
    self.GetTripAdvisorReviews = function () {
        $('#TripReviewModal' + self.Prop_Id()).modal('show');
    }
    ...
    self.VendorRate = ko.observable(data.Room_Standard_rate);
    self.RoomPrice = ko.observable(data.ActualRate);

    self.isShow = ko.observable(data.Room_Standard_rate > data.ActualRate);
