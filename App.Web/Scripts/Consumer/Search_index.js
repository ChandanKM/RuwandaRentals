...
subData = new SubscribeViewModel();
CheckSubscribeEmailLatter(subData);

function CheckSubscribeEmailLatter(subData) {
    $('#Subscribe').hide();
    $('#Unsubscribe').hide();
    if (Cons_Id != undefined && Cons_Id != "") {

        subData.Email = Cons_Id;
        ...
        $.ajax({
            type: "Post",
            url: '/api/Consumer/CheckSubscribeEmailLatter',
            ...
            success: function (response) {
                ...
                else if (response.ReturnMessage[0] == "0") {                    
                    $('#Email').show();
                    $('#Email').val("");
                    $('#Subscribe').show();
                }
