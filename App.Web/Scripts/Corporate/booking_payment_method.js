...
function BookingPaymentViewModel() {
    ...
    self.total_room_rate = ko.computed(function () {        
        return self.camo_room_rate() * parseInt(self.Room_Count()) * self.Day_Count();
    });

    self.tax_amnt = ko.computed(function () {
        return (self.ServiceTax() + self.LuxuryTax()) * parseInt(self.Room_Count()) * self.Day_Count();
    });
   
    self.net_amt = ko.computed(function () {
        return self.total_room_rate() + self.tax_amnt();
    });
