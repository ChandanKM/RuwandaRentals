...
function DayModel(data) {
    ...
    self.Room_Standard_rate = ko.observable(data.Room_Standard_rate || '0');
    ...
    self.changeRoom_Standard_rate = ko.observable(false);
    self.editRoom_Standard_rate = function () {
        self.changeRoom_Standard_rate(true);
    }

    self.SaveRoom_Standard_rate = function (eRow) {
        if (eRow.Room_Standard_rate() != '') {
            UpdateRoom_Standard_rate(eRow);
        }
        else {
            self.Room_Standard_rate('0');
            UpdateRoom_Standard_rate(eRow);
        }
    }
    ...
    self.ShowDetails = function (eRow) {
       roomrateVM.Getdetail(eRow)
    }
    self.HideLoaded = function () {
...
function DetailModel(data) {
    ...
    var self=this;
    self.vndramnt = data.vndr_amnt.toFixed(2);
    self.camflg_Amnt = data.camflg_Amnt.toFixed(2);
    self.tax_amnt = data.tax_amnt.toFixed(2);
    self.net_amt = data.net_amt.toFixed(2);
    self.ccavenuecharges = data.ccavenuecharges.toFixed(2);
    self.margin = data.margin.toFixed(2);
...
function RoomRateViewModel() {
    ...
    self.Getdetail  = function (eRow) 
    {
        $.ajax({
            type: "GET",
            url: "/api/Booking/CheckMargin",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            data: { Inv_Id: eRow.Inv_Id() },
            success: function (record) {
               
                self.Details.removeAll();
                
                for (var i = 0; i < record.Table.length; i++) {
                    self.Details.push(new DetailModel(record.Table[i]));
                }
            ...
            ko.applyBindings(roomrateVM, document.getElementById('modelShowDetail'));
...
function UpdateRoom_Standard_rate(data) {
    ...
    obj.Inv_Id(data.Inv_Id());
    obj.Available(data.Available());
    obj.Room_Standard_rate(data.Room_Standard_rate());

    $.ajax({
        type: "GET",
        url: "/api/rooms/updaterackPrice",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        data: { Inv_Id: obj.Inv_Id(), race_price: obj.Room_Standard_rate() },
        success: function (recorde) {
