...
var userTypeId = null;
authority_Id=4
function UserProfileModel(data, data1) {
    ...
    self.PropertyName = ko.observable(data1 || '');  // new Added for PropertyName
...
function UserProfileViewModel() {
    ...
    self.BindUserProfile = function () {
        ...
        $.ajax({
            ...
            contentType: "application/json; charset=utf-8",
            data: { AuthId: AuthId, UserId: '', PropId: PID },
            dataType: "json",
            success: function (data) {
                ...
                for (var i = 0; i < data.Table.length; i++) {
                    ...
                    self.UserProfileList.push(new UserProfileModel(data.Table[i], data.Table1[0].Prop_Name));
                }
                $('#PNAME').append(data.Table1[0].Prop_Name)
    ...
    OnSuccessSaveUserProfile = function (data) {
        ...
        for (var i = 0; i < data.Table1.length; i++) {
            self.UserProfileList.unshift(new UserProfileModel(data.Table1[i], $('#PNAME').text().trim()));
        }
        self.ResetForm();
    }
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (var i = 0; i < data.Table4.length; i++) {
                    self.ParamList.push(new ParamModel(data.Table4[i]));
                }
...
function ParamModel(data) {
    ...
    if (data.Vparam_Code == 'PC0001') {        
      ...
      //  $('#spnparm' + self.Id()).hide();
        ...
        if (data.Vparam_Code == 'PC0007')
        {
            self.shouldShowMessage(true);
        }
        self.PramType = ['MarkUp', 'MarkDown'];
...
function UpdateParametersValue(data) {
    ...
    if (data.Vparam_Val() >= 100) {
     
        var result = { Status: true, ReturnMessage: { ReturnMessage: "Not more then 99" }, ErrorType: "success" };
        Failed(result);
        objUserProfileVM.BindParam(null);
    }
    else if (data.Vparam_Val() < 0) {

        var result = { Status: true, ReturnMessage: { ReturnMessage: "Not less then 0" }, ErrorType: "success" };
        Failed(result);
        objUserProfileVM.BindParam(null);
    }
    else if (data.Vparam_Code() == 'PC0001' && data.Vparam_Val() >= 16) {
...
function PermissionModel(data, userId) {
    ...
    self.Active_flag = ko.observable(data.Active_flag == null ? false : true);
