    ...
    self.SaveUserProfile = function () {
        ...
        $.ajax({
            ...
            success: function (recorde) {
                ...
                var result = { Status: true, ReturnMessage: { ReturnMessage: 'Vendor Group Created Successfully' }, ErrorType: "Success" };
                Successed(result);
            },
            error: function (err) {
...
function UpdateParametersValue(data) {
    ...
    $.ajax({
        type: "GET",
        url: '/SuperAdmin/GetNoOfPropertiesCreated',
        ...
        success: function (dd) {
            ...
            if (dd > data.Vparam_Val()) {
                
                alert('Property Limit is less then the number of properties created')
...
function PermissionModel(data, userId) {
    ...
    self.Active_flag = ko.observable(data.Active_flag == null ? false : data.Active_flag.trim());
