...
function UserProfileViewModel() {
    ...
    self.BindUserProfile = function () {
        ...
        $.ajax({
            ...
            contentType: "application/json; charset=utf-8",
            data: { AuthId: AuthId, UserId: '', PropId: '' },
            dataType: "json",
    ...
    self.UpdateProfile = function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                var result = { Status: true, ReturnMessage: { ReturnMessage: 'Profile Updated Successfully' }, ErrorType: "Success" };
                Successed(result);
                $("#btnUpdate").hide();
            },
            error: function (err) {
  ...
  ko.cleanNode(objUserProfileVM.ParamList, document.getElementById("divParamGrid"));
       // AppCommonScript.ShowWaitBlock();
        $.ajax({
            ...
            contentType: "application/json; charset=utf-8",
            data: { propId: 0, authId: authority_Id },
            dataType: "json",
            async: false,
            success: function (data) {
    ...
    self.BindPermission = function (data) {
        ...
        $.ajax({
            type: "GET",
            url: "/api/UserParam/GetSuperAdminsPermission",
            contentType: "application/json; charset=utf-8",
            ...
            success: function (data) {
                ...
                for (var i = 0; i < data.Table.length; i++) {
                    self.PageList.push(new PermissionModel(data.Table[i], userId));
                }
...
function ParamModel(data) {
    ...
    self.Permissionselection = [ {"ShoweText":'YES',"ActualValue":'true'},{"ShoweText":'NO',"ActualValue":'false'}];
...
function UpdateParametersValue(data) {
    ...
    if (data.Vparam_Code() != "SC0001" && data.Vparam_Val() >= 16) {
...
function isNumber(evt) {
    ...
    if (charCode > 31 && (charCode < 48 || charCode > 57)) {
        if ($(evt.target.closest('tr')).find('td:eq(1)').text().trim() == "SC0001" && charCode == 46) {
            return true;
        }
        return false;
    }
...
function PermissionModel(data, userId) {
    ...
    self.Active_flag = ko.observable(data.Active_flag == "true      " ? true : false);
...
function UpdateActivePermission(data) {
    ...
    $.ajax({
        type: "Get",
        url: "/api/UserParam/UpdateSuperAdminPermissionFlag",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
